...
protected:
    ...
    AP_Float            _keep_servo_trim; // (0 or 1) if 0, then servos remain at trim pwm before the rotor startup sequence is executed, if set to 1 then servos function normally even before start up sequence is executed
    AP_Float            _scale_servo_output; // (1 ~ 3) Scale attitude output to servo to get larger servo response. Larger numbers means larger response. Limit scaling to factor of 3.
